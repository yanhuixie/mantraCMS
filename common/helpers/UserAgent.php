...
class UserAgent 
{
	 ...
	 * @return boolean
	 */
	public static function chkUnderIE11(){
		try{
		    $ua = static::parse_user_agent();
			if($ua && $ua['browser'] == "MSIE" && $ua['version'] < 11){
				return true;
			}
		}
		catch(\Exception $ex){
			Yii::warning("chkUnderIE11 failed.". $ex->getMessage());
			return false;
		}
		return false;
	}
	 ...
	 * @return boolean
	 */
	public static function chkMobile(){ //return true;
	    if(! isset($_SERVER['HTTP_USER_AGENT']) ) return false;
	    
		$agent = self::parse_user_agent();
		return in_array( $agent['platform'], ['Android', 'iPhone', 'iPad', 'Windows Phone'] ) ;
	}
